...
class BatteryListener(Node):
    def __init__(self):
        super()._init_('battery_log_listener')
    ...
    def battery_log_callback(self, msg):
        point = Point("battery").tag("robot_name", robot_name).field("percentage", msg.percentag).time(datetime.utcnow())
...
def main(arg=None):
    ...
    battery_log_listener = BatteryListener()
    rclpy.spin(battery_log_listener)
    battery_log_listener.destroy_node()
    rclpy.shutdown()
